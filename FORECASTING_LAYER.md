...
## LIVE RUNS 7 AND 8 — 22 August 2026 (v14) — findings

**Aakash stopped wiping `data/`**, and the fast clock immediately started
answering questions that had been guesswork for six runs.

### Three open questions closed by accumulated data

**The prior-work risk did not materialise.** Scaffolding moved numbers 14.2
points on average across **49 lens runs** — well under the 25-point threshold
where the flag fires. Working assumptions are not swamping each lens's own
work. That was the central objection to v14, and at n=49 it is retired.

**The thin warning works once the advice is removed.** Warned lenses moved 9.1
points off their base rate against 4.1 for unwarned (n=18 vs 17), and 14.2 vs
3.9 the run before. Run 6, with "count for MORE" still in, showed 11.2 vs 13.8
— the wrong direction. Deleting the clause fixed it.

**Fallbacks fell to 14%** (7 of 49), from 33% under v13 and 38% under v11.

**§4 executed for the first time.** "Q0001: no cause and not stale; number
persists flat", three times — the daily screen fired, gave reasons, and
correctly declined to re-forecast. That mechanism had never actually run.

### The lenses had been reading a third of the news — FIXED in v15

```
articles_considered=60      (164 distinct stories going forward)
```

`_news_digest` truncated at 60 articles in arbitrary order. **Roughly 63% of
the day's reporting never reached the screen or any inside view**, on every run
since the layer was built, and nothing said so because the count was printed as
a bare number.

**Fix:** `_news_for_question` selects the 40 most relevant stories per question
by embedding similarity instead of truncating a shared list. The machinery
already existed — stage one ranks per system the same way. Verified against the
exact failure: a cancelled-PM-visit story placed at position 164 is invisible
to the old truncation and ranks FIRST under selection. Ranker failure falls
back to truncation rather than dropping the question.

### The screen collapsed escalation into resolution — FIXED

> "Recent news mentions treaty renegotiations and a possible canceled PM visit,
> **but no formal renewal or expiration has occurred yet**." → `no_cause`

The clause after "but" answers the RESOLUTION question; the model then reused
it for ESCALATION. A cancelled prime-ministerial visit on a treaty-signing
question is the diagnosticity test passing — we would not see that story if a
signing were on track.

**Fix:** escalation is asked FIRST and on its own terms, with an explicit rule
that "the thing has not happened yet" is never a reason to decline escalation
(of course it has not — the question is still open), and a statement that the
two answers usually differ.

### Stale and undated papers entered as current reporting — FIXED

A **5 June** paper was read on a 22 August run, and a second came back "date
unknown" — together 137 articles, roughly half the input, feeding the screen and
every inside view as though they were that day's news.

Both filenames carried the correct date (`BS ● Delhi ● 22‹08‹2026 .pdf`,
`The Wall Street Journal Weekend - August 22, 2026.pdf`). The parser read only
page content, found a June date inside the first and nothing parseable in the
second.

**Fix:** the filename is parsed as a date source and WINS on disagreement — a
filename is chosen by a human who knows which issue it is, while page content
is full of dates belonging to other things. Numeric formats added. Disagreements
are reported, undated papers flagged, and any paper more than four days from the
run date is flagged before its articles are used. Verified against both real
filenames.

### Watch items

- **Extrapolation abuse is worsening**: `R0038-actor_incentive` claimed 60 cases
  from 2 named (30x), `R0036` 15 from 3, `R0032` 46 from 14. All caught by the
  3x ratio cap, concentrated in `actor_incentive` and `reference_class`.
- **Abstentions remain 0% at n=7 per lens** — approaching the point where small
  samples stop explaining it.
