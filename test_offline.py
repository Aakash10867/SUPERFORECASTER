...
def main() -> int:
    ...
    checks.append(("spread recorded on every forecast",
                   all(f["spread"] != "" for f in forecasts
                       if f["model"] == "aggregate")))
    checks.append(("reference_class decays on window questions",
                   any(r["p_one_third"] and r["p_two_thirds"] and
                       float(r["p_one_third"]) < float(r["p_two_thirds"])
                       for r in lens_rows if r["lens"] == "reference_class")))
