...
# Beyond this, a skeleton is not carrying the estimate -- it is decorating a
# guess. See audit_provenance().
MAX_EXTRAPOLATION_RATIO = 3.0

# A rate of 0 (or 1) built from a handful of cases is almost always a badly
# drawn population rather than a real finding -- and it propagates as a hard
# floor or ceiling into everything downstream. Live run 2 produced three
# zero-hit entries, one of them 0 of 2.
MIN_CASES_FOR_EXTREME_RATE = 8
...
def audit_provenance(payload: dict) -> tuple[str, str]:
    ...
    if not (skeleton and multiplier):
        return ("unsupported",
                f"claims {count} cases but only {n_cases} are named, with no "
                "skeleton or multiplier to justify the rest")

    # RATIO CAP. A skeleton is only honest extrapolation while the skeleton is
    # doing most of the work. On live run 2, R0016-analogy claimed 15 cases
    # from 3 named -- a 5x multiplier on a three-item list, where "extrapolated"
    # flatters what is really a guess with a decoration.
    ratio = count / n_cases
    if ratio > MAX_EXTRAPOLATION_RATIO:
        return ("unsupported",
                f"claims {count} cases from only {n_cases} named "
                f"({ratio:.1f}x); beyond {MAX_EXTRAPOLATION_RATIO}x the "
                "skeleton is not carrying the estimate")
    return ("extrapolated",
...
def build_entry(
    ...
    # Degrade an extreme rate built on too few cases. The entry is still
    # stored -- nothing is ever deleted -- but it is not allowed to present a
    # hard 0% or 100% as though it were established.
    extreme_note = ""
    if count and (hits == 0 or hits == count) and count < MIN_CASES_FOR_EXTREME_RATE:
        extreme_note = (
            f"rate of {'0' if hits == 0 else '100'}% from only {count} case(s); "
            "too few to establish an extreme base rate -- treat as directional"
        )
        if tier in ("enumerated", "extrapolated"):
            tier = "thin"
    ...
    return {
        ...
        "provenance_note": "; ".join(x for x in (note, extreme_note) if x),
