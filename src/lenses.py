...
{prior_work_block}{frozen_block}{scale_rule}
...
{{
  ...
  "prior_factors": [{{"name": "what this factor is", "belongs_to_me": false,
                     "value": 0, "basis": "one line, base rate only"}}],
  "combination": "how you combined them -- multiply, or something else, and why",
...
PRIOR_WORK_BLOCK = """PRIOR WORK -- READ THIS BEFORE YOU BEGIN.

{prior_work}

State every prior value in "prior_factors", marking which one is yours
("belongs_to_me": true) and which are working assumptions. Say in
"combination" how you put them together.

A caution on combining: multiplying assumes the factors are INDEPENDENT, and
they usually are not -- if the actors want something badly, friction is often
lower too. If multiplying would understate the answer, say so and combine
differently, but show your arithmetic either way.
...
{frozen_recall}{prior_recall}{thin_warning}
...
PRIOR_RECALL_BLOCK = """
YOUR PRIOR WORK, FIXED AT THE OUTSIDE STAGE AND NOT REVISABLE:
{factors}
  combined by: {combination}

These are yours -- you produced them, and using them is not a violation. But
they are FROZEN. Only the factor marked as your own may move now, and it moves
only on the evidence in front of you. If today's reporting bears on one of the
OTHER factors, that belongs to a different forecaster; note it and leave it
alone.
"""


THIN_WARNING_BLOCK = """
WARNING ABOUT YOUR OWN BASE RATE. The reference entry you built was judged
{tier} by the provenance check:

  {note}

A rate of 0% or 100% drawn from a handful of cases usually means the population
was drawn too narrowly, not that the outcome is certain.

State how much weight you are putting on that base rate, and why.
"""
# The original version ended by telling the lens that thin base rates mean the
# evidence in front of it "should count for MORE than usual". That was a
# mistake: a lens's aperture restricts WHICH evidence it may use, so an
# instruction to weigh evidence more heavily is an invitation to reach past the
# aperture. It also showed no measurable benefit -- lenses warned their base
# rate was thin moved 11.2 points off it against 13.8 for solid ones, the wrong
# direction. Verdict stated, advice removed.
...
PRIOR WORK -- THE MOST COMMON REASON AUDITS GO WRONG. READ CAREFULLY.
{prior_work}

The forecaster fixed working values for the factors outside its aperture BEFORE
seeing any news, and those values are frozen. Referring to them, restating
them, and doing arithmetic with them are all REQUIRED, not violations. If you
see something like "taking the 40% substantive estimate and applying 0.95 gives
38%", that is the forecaster following its instructions exactly.

So draw this distinction, because it is the whole job:

  ALLOWED -- an ASSUMPTION carried in from prior work:
    "assuming, as fixed earlier, that the authority wants this (80%)..."
    "my frozen substantive term was 40%, so 40 x 0.95 = 38"

  VIOLATION -- a CONCLUSION drawn here, from evidence:
    "recent reporting suggests the authority has cooled on this, so I lower..."
    "the deadline is tight, so I adjust downward"

The test is whether the forecaster REASONED ITS WAY to the value using evidence
in front of it, or simply USED a value it had already fixed. Only the first is
a violation.
...
class LensRunner:
    ...
    def __init__(self, router, log, settings, config_version, similarity=None,
                 shared_ground="", prior_work=""):
        self.shared_ground = shared_ground or ""
        self.prior_work = prior_work or ""
        self.router = router
        self.log = log
    ...
    def outside(self, lens, question, today, prior_entry=None):
        ...
        prompt = OUTSIDE_PROMPT.format(
            prior_work_block=(PRIOR_WORK_BLOCK.format(prior_work=self.prior_work)
                              if self.prior_work else ""),
    ...
    def reconcile(self, lens, question, today, outside, yes_case, no_case,
        ...
        prior_recall = ""
        factors = (outside or {}).get("prior_factors") or []
        if factors:
            lines = []
            for f in factors[:6]:
                if not isinstance(f, dict):
                    continue
                mine = " (YOURS -- may still move)" if f.get("belongs_to_me") else " (frozen)"
                lines.append(
                    f"  {f.get('name','')}: {f.get('value','')}{mine}"
                    f"  [{(f.get('basis') or '')[:90]}]"
                )
            if lines:
                prior_recall = PRIOR_RECALL_BLOCK.format(
                    factors="\n".join(lines),
                    combination=(outside.get("combination") or "unstated")[:200],
                )

        thin_warning = ""
        tier = (outside or {}).get("_provenance_tier", "")
        if tier in ("thin", "unsupported"):
            thin_warning = THIN_WARNING_BLOCK.format(
                tier=tier.upper(),
                note=(outside.get("_provenance_note") or "")[:300],
            )
