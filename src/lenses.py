...
{{
  ...
  "known_gaps": "...",
  "valid_until": "YYYY-MM-DD -- how long this base rate stays usable",
...
number, with the direction and rough size of the move. Not "if the political \
...
aperture",
  "triggers": [{{"event": "...", "direction": "up|down", "to_roughly": 0}}],
  {move_fields}
...
THE FORECASTER'S FORBIDDEN LIST:
{forbidden}
...
class LensRunner:
    ...
    def __init__(self, router, log, settings, config_version, similarity=None):
        ...
        self.grounding_enabled = bool(
            settings.get("reference", {}).get("verify_with_grounding", True)
        )
