...
ANY gate is eliminated, no matter how interesting it is.
  ...
  GATE F -- IT MUST BE YOUR DOMAIN. Does this question genuinely belong to
  {system_name}, or does it belong to one of these other domains?

{other_domains}

  Eliminate anything that belongs elsewhere, however good it is. That domain's
  own system is reading the same papers and will find it. A question proposed
  from the wrong domain either duplicates theirs and is discarded, or crowds
  out the question this domain should have contributed -- and the day's output
  is worse either way.

  GATE G -- IT MUST BE CURRENT. Papers reproduce their own old front pages and
  run retrospective features. If the question concerns something that reads as
  already settled, or refers to circumstances that do not match today's date,
  it was probably built on an archive reprint. Eliminate it.

  GATE E -- IT MUST MATTER. This is a macro forecasting system. Read the stated
  significance and judge it critically: are there at least TWO SPECIFIC THINGS,
  OUTSIDE this question's own subject, that would genuinely be different
  depending on the answer?

  If the only consequence of the answer is to the thing itself, eliminate it,
  however uncertain and however cleanly resolvable it is. A question can be
  perfectly forecastable and still be worthless.

  Apply this strictly. Things that are interesting to read about are not the
  same as things that matter. A single building, one company's product, a
  procedural step whose only effect is the next procedural step -- all fail,
  even when the reporting around them is dramatic.

  Do not accept the stated significance at face value. If it gestures at
  abstractions ("sets a precedent", "signals institutional tension") without
  naming what concretely changes, treat that as a failure.
...
{{
  ...
  "eliminated": [
    {{"proposal_id": "...", "outcome": "failed_gate" or "lost_in_system", "reason": "<short, specific -- name which gate if it failed one>"}}
  ]
}}
