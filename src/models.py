...
class ModelRouter:
        ...
        *,
        ...
        for model in chain:
            ...
            for attempt in range(self._max_retries + 1):
                ...
                ok, payload, err = self._post(model, prompt, temperature, max_output_tokens)
    ...
    def embed(self, texts: list[str]) -> list[list[float]] | None:
        """
        Embed a list of strings. Returns None if embeddings are unavailable,
        in which case callers fall back to local lexical similarity.
        """
        ...
        for model in chain:
            ...
            for text in texts:
                self._throttle(model)
                url = f"{API_ROOT}/{model}:embedContent"
                body = {
                ...
                self.stats.total_calls += 1
                if r.status_code != 200:
                    self.stats.failures_by_model[model].append(f"embed-http-{r.status_code}")
