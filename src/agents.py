...
DOMAINS THAT ARE NOT YOURS -- other systems own these, and they are reading the
same papers you are:
{other_domains}

STAY INSIDE YOUR DOMAIN. If the best story you can see belongs to another
domain, LEAVE IT ALONE. That domain's own system will find it. A question
proposed from the wrong domain either duplicates theirs and is thrown away, or
crowds out the question they should have won with -- and the day's output is
worse for it.

YOUR HUNT -- this is the only shape of question you look for:
{hunt}

Other agents are hunting other shapes within your domain. Do not stray outside
yours. If today's articles contain nothing of your shape AND your domain,
return an empty array. Producing nothing is a perfectly good outcome and far
better than forcing a weak question or reaching into someone else's territory.

BEWARE HISTORICAL REPRINTS. Papers reproduce their own old front pages and run
retrospective features. If an article describes something that reads as already
settled, or refers to officials, prices or circumstances that do not match
today's date of {today}, it is probably an archive reprint. Do not build a
question on it -- you would be forecasting something that happened years ago.
...
6. IT MUST MATTER. This is a MACRO forecasting system. The answer has to change
   something in the world beyond the question's own subject.

   The test: name at least TWO SPECIFIC THINGS, outside the immediate subject,
   that would be different depending on the answer. If the only consequence of
   the answer is to the thing itself, the question fails and you must not
   propose it.

   Passes: a central bank rate decision (borrowing costs for millions, currency,
   bank margins). A rule opening protected land (an industry, litigation
   precedent, climate policy). A leaders' summit (alliance posture, sanctions,
   regional security).

   Fails: whether one building gets finished. A single company's product launch.
   A procedural step whose only effect is the next procedural step. These can be
   perfectly uncertain and perfectly resolvable and still be worthless to
   forecast, because nothing turns on the answer.

   Be strict. A question that is interesting to read about is not the same as a
   question that matters.

7. GOLDILOCKS. The question must be genuinely uncertain to a well-informed
...
- "significance": the two or more specific things OUTSIDE this question's own
  subject that would be different depending on the answer. Name them concretely.
  Do not assert that it matters -- show what changes.
...
def run_agent(router, system_key, system_cfg, shape_key, shape_cfg,
    ...
    prompt = PROMPT.format(
        ...
        other_domains=other_domains or "(none)",
    ...
    for item in result:
        ...
        # Same logic for significance. If the agent cannot name what changes in
        # the world depending on the answer, the honest conclusion is that
        # nothing does.
        significance = str(item.get("significance", "")).strip()
        if len(significance.split()) < 8:
...
def relevant_articles(articles, system_key: str, min_before_fallback: int = 12):
    """
    Articles in scope for this system.

    Triage tags each article with the domains it could belong to. Articles it
    could not classify are a judgement call: including them everywhere means a
    story triage failed to label still gets a chance, but it also floods every
    system with the same material. The first live run showed the cost of being
    too generous -- the global-macro system was handed 88 articles and proposed
    a question about an Indian stock exchange IPO.

    So: take the tagged articles, and fall back to untagged ones only when a
    system has too little to work with.
    """
    tagged, untagged = [], []
    for a in articles:
        ...
        if not domains:
            untagged.append(a)
        elif system_key in domains:
            tagged.append(a)

    if len(tagged) >= min_before_fallback:
        return tagged
    return tagged + untagged


def dedupe_proposals(proposals, sim, threshold, log):
    """
    Collapse proposals that are the same question in different words.

    The first live run exposed a flaw in the agent design. The four shapes were
    meant to partition the news, but a single story can genuinely fit all of
    them at once: an RBI proposal to ban flexi loans is simultaneously a
    scheduled decision, a deadline under strain, a threshold and an announced
    intention. So four agents read one story and produced four wordings of one
    question, and the contest judge was handed nine candidates that were really
    three ideas.

    Collapsing them here means the judge compares genuinely different options.
    Agreement between agents is kept as a note -- several agents converging on
    one story is weak evidence it matters -- but it never counts as three
    separate candidates.
    """
    if len(proposals) < 2:
        return proposals

    texts = [p.question for p in proposals]
    ...
    for i, p in enumerate(proposals):
        ...
        for j in range(i + 1, len(proposals)):
            ...
            if score(i, j) >= threshold:
                group.append(proposals[j])
                used.add(j)

        # Keep the one with the most concrete reasoning, not simply the first.
        winner = max(group, key=lambda x: len(x.reasoning_value or ""))
        if len(group) > 1:
            agents = sorted({g.agent for g in group})
            winner.outcome_reason = (
                f"also proposed by: {', '.join(a for a in agents if a != winner.agent)}"
            )
            for g in group:
                if g is not winner:
                    g.outcome = "duplicate"
                    g.outcome_reason = f"same question as {winner.proposal_id}"
                    dropped += 1
        kept.append(winner)

    if dropped:
        log.info(f"  collapsed {dropped} duplicate proposal(s) -> {len(kept)} distinct")
    return kept
