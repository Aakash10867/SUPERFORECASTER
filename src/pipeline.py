...
from .agents import Proposal, relevant_articles, run_agent
...
def run(today: dt.date | None = None, dry_run: bool = False) -> int:
    ...
    for system_key, system_cfg in (agents_cfg.get("systems") or {}).items():
        ...
        winner, exceptional = run_contest(
            router, system_key, system_cfg, system_proposals,
            papers_desc, today, log,
        )
