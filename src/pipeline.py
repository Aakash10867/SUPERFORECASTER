...
def _run_forecasting(router, sim, settings, today, log, dry_run, news_text):
    ...
    runner = lensmod.LensRunner(router, log, settings, config_version, sim)
    ...
    for q in open_qs:
        ...
        if not shape:
            # Stage one does not yet emit `shape`. Default to point, because
            # the failure modes are asymmetric: calling a window a point loses
            # a test, while calling a point a window invents numbers that
            # correspond to nothing.
            shape = "point"
            if not dry_run:
                store.update_question(qid, {"shape": shape})
            log.info(f"  {qid}: no shape recorded; defaulting to 'point'. "
                     "Set it in questions.csv if this is a window question.")
        ...
        if record.get("status") == "forecast":
            log.info(
                ...
                f"{record.get('probability')} "
                f"({record.get('responding_lenses')} lenses responded, "
                f"{record.get('abstained_lenses')} abstained)"
            )
            adv = record.get("advocate") or {}
            if adv.get("load_bearing_assumption"):
                log.info(f"  advocate: {adv['load_bearing_assumption'][:300]}")
        if not dry_run:
