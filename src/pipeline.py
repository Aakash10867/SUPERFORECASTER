...
def run(today: dt.date | None = None, dry_run: bool = False,
    ...
    try:
        ...
        if "resolution" in wanted:
            try:
                _run_resolution(router, settings, today, log, dry_run,
                                news_text, articles, sim)
...
def _read_inbox(router, settings, today, log, dry_run):
    ...
    for path in pdfs:
        ...
        # STALE OR UNDATED PAPERS ARE ANNOUNCED, NOT SWALLOWED.
        #
        # On live run 8 a paper was read as 5 JUNE on a 22 August run, and
        # another came back "date unknown" -- together 137 articles, roughly
        # half the input, entering the screen and every inside view as though
        # they were that day's reporting. The prompts say "reporting since the
        ...
        if not paper.issue_date_guess:
            log.flag(
                ...
                f"filename or the pages. Its {len(paper.pages)} pages will be "
                "treated as current reporting. Put the date in the filename "
                "(e.g. 'WSJ - August 22, 2026.pdf') if that is wrong."
            )
        else:
            try:
                issued = dt.date.fromisoformat(paper.issue_date_guess)
                age = (today - issued).days
                if age > 4 or age < -1:
                    log.flag(
                        f"{path.name} is dated {paper.issue_date_guess}, "
                        f"{age} days from this run. Its articles will be fed "
                        "to the screen and to every lens as CURRENT reporting. "
                        "Remove it from the inbox if that is not what you want."
                    )
            except ValueError:
                pass
...
def _news_digest(articles, limit: int = 60) -> str:
    """
    A compact digest of everything read this run.
    ...
    NOTE: this is the UNTARGETED digest, used only where no question is in
    hand. For anything question-specific use _news_for_question, which selects
    rather than truncates -- see the note there.
    """
    if not articles:
        return ""
    return "\n\n---\n\n".join(_article_text(a) for a in articles[:limit])


def _news_for_question(question, articles, sim, log, limit: int = 40) -> str:
    """
    The reporting most relevant to ONE question.

    THE BUG THIS FIXES
    ------------------
    Every news-driven judgement in the system -- the daily screen and both
    inside views of all seven lenses -- was fed the FIRST 60 articles of an
    arbitrarily ordered list. On live run 8 that was 60 of 164 distinct
    stories: roughly 63% of the day's reporting never reached any lens, and
    nothing in the log said so, because the count was printed as a bare number.

    That silently capped the quality of every inside view, and it is the most
    likely reason the Q0003 screen saw only a passing mention of a cancelled
    prime-ministerial visit on a treaty-signing question.

    Selecting beats truncating, and the machinery already exists: stage one
    ranks articles per system with embeddings. This does the same per question.
    """
    if not articles:
        return ""
    if len(articles) <= limit:
        return _news_digest(articles, limit)

    query = " ".join([
        question.get("question", ""),
        question.get("primary_tag", "") or "",
        (question.get("resolution_criteria", "") or "")[:300],
    ])
    texts = [_article_text(a) for a in articles]
    try:
        ranked = sim.rank(query, texts)
    except Exception:                                      # noqa: BLE001
        # Ranking is an optimisation, never a blocker: fall back to the old
        # truncation rather than dropping the question.
        log.info("    article ranking unavailable; using the first "
                 f"{limit} stories")
        return _news_digest(articles, limit)

    chosen = [texts[i] for i, _score in ranked[:limit]]
    return "\n\n---\n\n".join(chosen)
...
def _run_resolution(router, settings, today, log, dry_run, news_text,
    ...
    for q in open_qs:
        qid = q.get("id", "")
        q_news = _news_for_question(q, articles, sim, log)
        result = resolve.screen_question(
            q, q_news, _trigger_text(qid),
        ...
        if not dry_run:
            store.append_row(config.SCREENS_CSV, {
                ...
                "articles_considered": len(q_news.split("---")) if q_news else 0,
        ...
        if result.get("resolution_nominee"):
            ...
            ok, outcome, event_date, detail = resolve.confirm_resolution(
                q, result.get("resolution_event", ""), q_news, router, log
            )
    ...
    if watched:
        ...
        for q in watched:
            result = resolve.screen_question(
                q, _news_for_question(q, articles, sim, log), "",
                since="the last run", today=today, router=router,
            )
...
def _run_forecasting(router, sim, settings, today, log, dry_run, news_text,
    ...
    runner = lensmod.LensRunner(
        ...
        prior_work=lenses_cfg.get("prior_work", ""),
    )
    ...
    for q in open_qs:
        ...
        record = forecast.run_question(
            ...
            _news_for_question(q, articles, sim, log), cause, similarity=sim,
