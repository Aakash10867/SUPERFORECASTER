...
ANSWER JOB 1 FIRST, AND ANSWER IT ON ITS OWN TERMS.

JOB 1 -- ESCALATION. Does anything here bear on the question enough to justify \
re-running the full forecast?

  The test is DIAGNOSTICITY: would we be seeing this story if the answer \
turned out to be the opposite? Reporting that is equally likely either way \
carries no information however alarming the headline -- and reporting that is \
much more likely under one answer than the other matters even if it is buried \
on an inside page.

  A restatement of a story already reported is not new. Everything else that \
bears on the question is.

  CRITICAL: "the thing has not happened yet" is NEVER a reason to decline \
escalation. Of course it has not happened yet -- the question is still open. \
Escalation asks whether the WORD ON ITS LIKELIHOOD has changed, not whether it \
has occurred. A cancelled meeting, a stalled negotiation, a new objection, a \
scheduling change: none of these resolve anything, and all of them are \
reasons to re-forecast.

JOB 2 -- RESOLUTION, a separate question with a much higher bar. Has a \
DEFINITIVE REPORTED ACT occurred that meets the resolution criteria, in EITHER \
direction? Not "it looks likely" -- a specific reported event. If nothing \
definitive has been reported, say no. This is only a nomination; a separate \
check will confirm it.

Your answer to Job 2 must not influence your answer to Job 1. They usually \
differ: most days something bears on the question and nothing resolves it.
...
{{
  "escalate": true/false,
  "escalation_reason": "what changed, and why it is diagnostic",
