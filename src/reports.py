...
def scaffold_dominance(log) -> dict:
    """
    How much of each lens's number came from its OWN factor, and how much from
    the working assumptions it had to make about everyone else's?

    Prior work solves a real bind -- a lens cannot turn "30% chance of a block"
    into a whole-question probability without assuming something about will and
    schedule. But if the assumptions move the number further than the lens's
    own study does, we have replaced visible leakage with invisible guesswork,
    which is worse. This makes that measurable.
    """
    rows = store.read_rows(config.LENS_CSV)
    per_lens = {}
    for r in rows:
        try:
            shift = abs(float(r.get("scaffold_shift")))
        except (TypeError, ValueError):
            continue
        per_lens.setdefault(r.get("lens", ""), []).append(shift)
    out = {k: {"n": len(v), "mean_shift": round(statistics.mean(v), 1)}
           for k, v in per_lens.items() if v}
    allv = [x for v in per_lens.values() for x in v]
    if allv:
        out["_all"] = {"n": len(allv), "mean_shift": round(statistics.mean(allv), 1)}
    return out


def thin_base_rate_effect(log) -> dict:
    """
    When a lens is told its own base rate is thin, does it move further off it?

    The thin-rate label was inert until v13 -- it sat in a JSON field the lens
    never read. This measures whether telling the lens actually changes
    anything, which is the only way to know the warning is worth its tokens.
    """
    rows = store.read_rows(config.LENS_CSV)
    warned, normal = [], []
    for r in rows:
        try:
            d = abs(float(r.get("inside_drift")))
        except (TypeError, ValueError):
            continue
        (warned if r.get("provenance_tier") in ("thin", "unsupported")
         else normal).append(d)
    out = {}
    if warned:
        out["thin"] = {"n": len(warned), "mean_abs_drift": round(statistics.mean(warned), 1)}
    if normal:
        out["solid"] = {"n": len(normal), "mean_abs_drift": round(statistics.mean(normal), 1)}
    return out
...
def write_reports(today: dt.date, log, settings) -> None:
    ...
    scaffold = scaffold_dominance(log)
    thin_fx = thin_base_rate_effect(log)
    ...
    payload = {
        ...
        "advocate_drift": adv,
        "thin_base_rate_effect": thin_fx,
        "scaffold_dominance": scaffold,
        "inside_drift": drift,
        "fallback_bias": fb,
        "scoring": summary,
    ...
    if scaffold.get("_all"):
        a = scaffold["_all"]
        log.sub("Prior work: own factor vs scaffolding")
        log.info(
            f"  across {a['n']} lens runs, the working assumptions moved the "
            f"number {a['mean_shift']} points away from the factor the lens "
            "actually studied."
        )
        if a["n"] >= 15 and a["mean_shift"] >= 25:
            log.flag(
                "The assumptions a lens makes about OTHER lenses' territory are "
                "moving its number further than its own work does. That is "
                "invisible guesswork replacing visible leakage, which is worse. "
                "Reconsider whether these lenses should be separate apertures."
            )

    if thin_fx.get("thin") and thin_fx.get("solid"):
        t, sol = thin_fx["thin"], thin_fx["solid"]
        log.sub("Thin base rates")
        log.info(
            f"  when a lens was told its base rate was thin it moved "
            f"{t['mean_abs_drift']} points off it on average ({t['n']} runs); "
            f"when the base rate was solid, {sol['mean_abs_drift']} "
            f"({sol['n']} runs)."
        )
        if t["n"] >= 10 and t["mean_abs_drift"] <= sol["mean_abs_drift"]:
            log.info(
                "  NOTE: the warning is not changing behaviour. Either the "
                "lenses are ignoring it, or thin base rates are not actually "
                "the problem."
            )
